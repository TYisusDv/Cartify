...
const CrudPage: React.FC<CrudPageProps> = ({ addAlert, onClose, handleTableReload, setSelected, type, selected_id }) => {
    ...
    const handleForm = async () => {
        if (type === 'add') return addPaymentMethod(formValues);
        if (type === 'edit') return editPaymentMethod(formValues);
        if (type === 'delete' && selected_id) return deletePaymentMethod(selected_id);
    };
