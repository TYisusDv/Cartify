...
import { Add01Icon, DashboardBrowsingIcon, Delete02Icon, PencilEdit02Icon } from 'hugeicons-react';
...
import { getCountSaleStatus } from '../../../../services/SaleStatus';
...
const Status: React.FC = () => {
    ...
    const table_header = [
        { name: 'name', headerName: 'Nombre' },
        { name: 'calculate', headerName: 'Permite calcular' },
    ];

    useEffect(() => {
        const fetchCount = async () => {
            try {
                const response = await getCountSaleStatus();
        ...
        <DelayedSuspense fallback={<SkeletonLoader />} delay={1000}>
            <div className='flex items-center justify-between w-full p-8 animate__animated animate__fadeIn animate__faster'>
                <div className='flex flex-col'>
                    <h1 className='text-2xl font-bold dark:text-white'>{translations.sale_status}</h1>
                    <span className='text-sm text-gray-600 dark:text-slate-400'>{translations.manage_sale_status}</span>
                </div>
            ...
            <div className='flex flex-col p-8 animate__animated animate__fadeIn animate__faster'>
                ...
                <div className='w-full mt-6'>
                    <Table endpoint='manage/sale/status' reloadTable={reloadTable} header={table_header} tbody={<TablePage selected={selected} setSelected={setSelected} />} />
                </div>
            </div>
            {isModalOpen.add && (
                <Modal title={translations.add_sale_status} onClose={() => setIsModalOpen({ ...isModalOpen, add: false })}>
            ...
            {isModalOpen.edit && (
                <Modal title={translations.edit_sale_status} onClose={() => setIsModalOpen({ ...isModalOpen, edit: false })}>
            ...
            {isModalOpen.delete && (
                <Modal title={translations.delete_sale_status_sure} onClose={() => setIsModalOpen({ ...isModalOpen, delete: false })}>
