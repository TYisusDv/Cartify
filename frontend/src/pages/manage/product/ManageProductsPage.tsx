...
const ManageProductsPage: React.FC<ManageProductsPageProps> = ({ addAlert }) => {
    ...
    const [isModalOpen, setIsModalOpen] = useState({ add: false, edit: false, delete: false, details: false, add_brand: false, add_category: false, add_supplier: false });
    ...
    const toggleModal = (modalType: 'add' | 'edit' | 'delete' | 'details' | 'add_brand' | 'add_category' | 'add_supplier', isOpen: boolean) => {
        ...
        <DelayedSuspense fallback={<SkeletonLoader />} delay={1000}>
            ...
            <div className='flex flex-col p-8 animate__animated animate__fadeIn animate__faster'>
                ...
                <div className='w-full mt-6'>
                    <Table endpoint='manage/products' reloadTable={reloadTable} header={table_header} tbody={<TablePage selected={selected} setSelected={setSelected} />} />
                </div>
            </div>
            {isModalOpen.add && (
            ...
            {isModalOpen.edit && (
                <Modal title={translations.edit_product} onClose={() => toggleModal('edit', false)}>
                    <CrudPage addAlert={addAlert} type='edit' selected_id={selected} onClose={() => toggleModal('edit', false)} handleTableReload={handleTableReload} setSelected={setSelected} toggleModal={toggleModal} />
                </Modal>
            )}

            {isModalOpen.delete && (
                <Modal title={translations.delete_product} onClose={() => toggleModal('delete', false)}>
                    <CrudPage addAlert={addAlert} type='delete' selected_id={selected} onClose={() => toggleModal('delete', false)} handleTableReload={handleTableReload} setSelected={setSelected} toggleModal={toggleModal} />
                </Modal>
            )}

            {isModalOpen.details && (
                <Modal title={translations.details_product} onClose={() => toggleModal('details', false)}>
                    <CrudPage addAlert={addAlert} type='details' selected_id={selected} onClose={() => toggleModal('details', false)} handleTableReload={handleTableReload} setSelected={setSelected} toggleModal={toggleModal} />
                </Modal>
            )} 
