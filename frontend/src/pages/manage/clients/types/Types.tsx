...
import { Add01Icon, Delete02Icon, PencilEdit02Icon, UserAccountIcon } from 'hugeicons-react';
...
import { getCount } from '../../../../services/ClientTypes';
        ...
        <DelayedSuspense fallback={<SkeletonLoader />} delay={1000}>
            <div className='flex items-center justify-between w-full p-8 animate__animated animate__fadeIn animate__faster'>
                <div className='flex flex-col'>
                    <h1 className='text-2xl font-bold dark:text-white'>{translations.client_types}</h1>
                    <span className='text-sm text-gray-600 dark:text-slate-400'>{translations.manage_client_types_info}</span>
                </div>
            ...
            <div className='flex flex-col p-8 animate__animated animate__fadeIn animate__faster'>
                ...
                <div className='w-full mt-6'>
                    <Table endpoint='manage/clienttypes' reloadTable={reloadTable} header={table_header} tbody={<TablePage selected={selected} setSelected={setSelected} />} />
                </div>
            </div>
            {isModalOpen.add && (
                <Modal title={translations.add_client_type} onClose={() => toggleModal('add', false)}>
            ...
            {isModalOpen.edit && (
                <Modal title={translations.edit_client_type} onClose={() => toggleModal('edit', false)}>
            ...
            {isModalOpen.delete && (
                <Modal title={translations.delete_client_type_sure} onClose={() => toggleModal('delete', false)}>
