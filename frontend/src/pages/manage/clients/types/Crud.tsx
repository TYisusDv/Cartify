...
import { addClientType, deleteClientType, editClientType, getClientType } from '../../../../services/ClientTypes';
...
const Crud: React.FC<CrudProps> = ({ onClose, handleTableReload, setSelected, type, selected_id }) => {
    ...
    const [formValues, setFormValues] = useState<ClientType>({ id: selected_id });
    ...
    useEffect(() => {
        ...
        if ((type === 'details' || type === 'delete' || type === 'edit') && selected_id) {
            const fetchGet = async () => {
                try {
                    const response = await getClientType(formValues);
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let response_resp;
            if (type === 'add') {
                const response = await addClientType(formValues);
                response_resp = response?.resp;
            } else if (type === 'edit') {
                const response = await editClientType(formValues);
                response_resp = response?.resp;
            } else if (type === 'delete' && selected_id) {
                const response = await deleteClientType(formValues);
                response_resp = response?.resp;
            }
