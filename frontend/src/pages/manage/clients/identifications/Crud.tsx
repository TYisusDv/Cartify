...
import { addTypeID, deleteTypeID, editTypeID, getTypeID } from '../../../../services/TypesIDs';
...
const Crud: React.FC<CrudProps> = ({ onClose, handleTableReload, setSelected, type, selected_id }) => {
    ...
    const [formValues, setFormValues] = useState<TypeID>({ id: selected_id, status: true });
    ...
    useEffect(() => {
        ...
        if ((type === 'details' || type === 'delete' || type === 'edit') && selected_id) {
            const fetchGet = async () => {
                try {
                    const response = await getTypeID(formValues);
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let response_resp;
            if (type === 'add') {
                const response = await addTypeID(formValues);
                response_resp = response?.resp;
            } else if (type === 'edit') {
                const response = await editTypeID(formValues);
                response_resp = response?.resp;
            } else if (type === 'delete' && selected_id) {
                const response = await deleteTypeID(formValues);
                response_resp = response?.resp;
            }
