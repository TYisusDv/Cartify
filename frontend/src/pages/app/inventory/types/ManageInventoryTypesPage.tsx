...
const ManageInventoryTypesPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchCount = async () => {
            try {
                const response = await getCountInventoryTypes();
        ...
        <DelayedSuspense fallback={<SkeletonLoader />} delay={1000}>
            <div className='flex items-center justify-between w-full p-8 animate__animated animate__fadeIn animate__faster'>
                <div className='flex flex-col'>
                    <h1 className='text-2xl font-bold dark:text-white'>{translations.inventory_types}</h1>
                    <span className='text-sm text-gray-600 dark:text-slate-400'>{translations.manage_inventory_types_info}</span>
                </div>
            ...
            <div className='flex flex-col p-8 animate__animated animate__fadeIn animate__faster'>
                ...
                <div className='w-full mt-6'>
                    <Table endpoint='manage/inventory/types' reloadTable={reloadTable} header={table_header} tbody={<TablePage selected={selected} setSelected={setSelected} />} />
                </div>
            </div>
            {isModalOpen.add && (
                <Modal title={translations.add_inventory_type} onClose={() => toggleModal('add', false)}>
            ...
            {isModalOpen.edit && (
                <Modal title={translations.edit_inventory_type} onClose={() => toggleModal('edit', false)}>
            ...
            {isModalOpen.delete && (
                <Modal title={translations.delete_inventory_type} onClose={() => toggleModal('delete', false)}>
...
export default ManageInventoryTypesPage;