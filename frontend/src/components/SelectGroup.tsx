...
const SelectGroup: React.FC<SelectGroupProps> = ({ options, value, onChange }) => {
  ...
  return (
    <div className="relative min-w-32">
      ...
      {isOpen && (
        <ul className="absolute left-0 top-full mt-1 w-full border border-gray-200 rounded-md bg-white dark:bg-slate-700 dark:text-white dark:border-slate-600 z-10">
          {options.map((option) => (
