...
class ManageGuaranteesAPIView(APIView):
    ...
    def get_object(self, pk) :
        try:
            return GuaranteesModel.objects.get(pk = pk)
        except GuaranteesModel.DoesNotExist:
            raise Http404('Warranty not found.')
    ...
    def get(self, request):
        ...
        if query == 'table':
            ...
            model = GuaranteesModel.objects.filter(query_terms)
            ...
            serialized = GuaranteesSerializer(model, many = True)
            ...
            serializer = GetWarrantySerializer(data = data)  
            if not serializer.is_valid():
                return JsonResponse({
                    'success': False, 
                    'resp': serializer.errors
            ...
            serialized = GuaranteesSerializer(instance)
            ...
            model = GuaranteesModel.objects.filter(
                Q(id__icontains = search)
            )[:10]
            serialized = GuaranteesSerializer(model, many = True)
            ...
            total = GuaranteesModel.objects.filter().count()            
    ...
    def post(self, request):
        ...
        serializer = AddEditWarrantySerializer(data = data)
        if not serializer.is_valid():
            return JsonResponse({'success': False, 'resp': serializer.errors}, status = 400)
    ...
    def put(self, request):
        ...
        serializer = AddEditWarrantySerializer(instance, data = data)
        if not serializer.is_valid():
            return JsonResponse({'success': False, 'resp': serializer.errors}, status = 400)
...
class ManageUsersAPIView(APIView):
    ...
    def get(self, request):
        ...
        if query == 'table':
            ...
            model = User.objects.filter(query_terms)
            ...
            serializer = GetUserSerializer(data = data)  
            if not serializer.is_valid():
                return JsonResponse({
                    'success': False, 
                    'resp': serializer.errors
...
class ManageAbsencesAPIView(APIView):
    ...
    def get_object(self, pk) :
        try:
            return AbsencesModel.objects.get(pk = pk)
        except AbsencesModel.DoesNotExist:
            raise Http404('Absence not found.')
    ...
    def get(self, request):
        ...
        if query == 'table':
            ...
            model = AbsencesModel.objects.filter(query_terms, user_id = data_id)
            ...
            serialized = AbsencesSerializer(model, many = True)
            ...
            serialized = AbsencesSerializer(instance)
            ...
            model = AbsencesModel.objects.filter(
                Q(id__icontains = search) |
                Q(name__icontains = search)
            )[:10]
            serialized = AbsencesSerializer(model, many = True)
    ...
    def post(self, request):
        ...
        serializer = AddEditAbsenceSerializer(data = data)
        if not serializer.is_valid():
            return JsonResponse({'success': False, 'resp': serializer.errors}, status = 400)
    ...
    def put(self, request):
        ...
        serializer = AddEditAbsenceSerializer(instance, data = data)
        if not serializer.is_valid():
            return JsonResponse({'success': False, 'resp': serializer.errors}, status = 400)
